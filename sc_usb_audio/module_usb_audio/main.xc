...
#define c_EANativeTransport_ctrl null
    ...
    par
    {
        ...
        {
            thread_speed();
            Endpoint0( c_xud_out[0], c_xud_in[0], c_aud_ctl, null, c_clk_ctl, c_EANativeTransport_ctrl, dfuInterface);
        }
...
#endif
buffered in port:1 rx = on tile[0] : XS1_PORT_1M;
...
#define c_clk_ctl null
    ...
    par
    {
        ...
        {
            par{
                uart_rx(rx, rx_buffer, ARRAY_SIZE(rx_buffer), BAUD_RATE, 8, UART_RX_PARITY_NONE, 1, chanRX);
            }
        }
        on tile[0] :
        {
        ...
        {
            thread_speed();
            usb_midi(p_midi_rx, p_midi_tx, clk_midi, c_midi, 0, c_iap, null, null, null,chanRX);
        }
#else
#if defined(MIDI)
        /* MIDI core */
