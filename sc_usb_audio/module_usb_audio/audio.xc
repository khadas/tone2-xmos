    ...
    chanend ?c_adc)
{
    ...
    int dsdCount = 0;
    int everyOther = 1;
#if 1  // support multichannel 20220225
    unsigned dsdSample[DSD_CHANS_DAC];
#else
    ...
    adatCounter = 0;
#endif
#if (DSD_CHANS_DAC != 0) // support multichannel 20220225
    for(int i=0; i < DSD_CHANS_DAC; i++)
    {
       dsdSample[i] = 0x96960000;
    }
#endif
#if(DSD_CHANS_DAC != 0)
    if(dsdMode == DSD_MODE_DOP)
    ...
    while (1)
    {
        ...
        if(dsdMode == DSD_MODE_NATIVE)
        {
...
#else       // support multichannel 20220225
            for(int i=0; i < DSD_CHANS_DAC; i++)
            {
               asm volatile("out res[%0], %1"::"r"(p_dsd_dac[i]),"r"(bitrev(byterev(samplesOut[i]))));
            }
            /* Output DSD data to ports then 32 clocks */
#endif
#ifndef __XS2A__
            ...
            {
                case 4:
#if 0       // support multichannel 20220225
                    asm volatile("out res[%0], %1"::"r"(p_dsd_dac[0]),"r"(dsdSample_l));
                    asm volatile("out res[%0], %1"::"r"(p_dsd_dac[1]),"r"(dsdSample_r));
#endif
...
// SPDIF-DOP
            ...
            {
#if 1       // support multichannel 20220225
                dsdSample[0] = ((samplesOut[0] & 0xffff00) << 8);
                dsdSample[1] = ((samplesOut[1] & 0xffff00) << 8);
                /*for(int i=0; i < DSD_CHANS_DAC; i++)
                {
                   dsdSample[i] = ((samplesOut[i] & 0xffff00) << 8);
                }*/
#else
            ...
            else // everyOther
            {
                ...
                for(int i=0; i < DSD_CHANS_DAC; i++)
                {
                   dsdSample[i] = (dsdSample[i] | ((samplesOut[i] & 0xffff00) >> 8));
                }
#endif
...
#else       // support multichannel 20220225
                asm volatile("out res[%0], %1"::"r"(p_dsd_dac[0]),"r"(bitrev(dsdSample[0])));
                asm volatile("out res[%0], %1"::"r"(p_dsd_dac[1]),"r"(bitrev(dsdSample[1])));
#if DSD_CHANS_DAC > 2
                asm volatile("out res[%0], %1"::"r"(p_dsd_dac[2]),"r"(bitrev(dsdSample[2])));
                asm volatile("out res[%0], %1"::"r"(p_dsd_dac[3]),"r"(bitrev(dsdSample[3])));
                asm volatile("out res[%0], %1"::"r"(p_dsd_dac[4]),"r"(bitrev(dsdSample[4])));
                asm volatile("out res[%0], %1"::"r"(p_dsd_dac[5]),"r"(bitrev(dsdSample[5])));
#endif
#endif
#ifndef __XS2A__
                switch (divide)
